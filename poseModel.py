...
# Calculate angles for each row in the dataset
# landmarks_points = {}  # A dictionary to store landmark points
# pose_angles = df.apply(lambda row: rangles(pd.DataFrame([row]), landmarks_points), axis=1)

# # Flatten angles into columns and add them to the dataframe
# angles_df = pd.DataFrame(pose_angles.tolist(), columns=[
